...
fn swash_image(
    ...
    if wght.is_some() || opsz.is_some() {
        let settings = wght.into_iter().chain(opsz).map(|s| (s.tag, s.value));
        scaler = scaler.normalized_coords(swash_font.variations().normalized_coords(settings));
    }
...
mod test {
    ...
    fn no_coord_leakage_across_fonts() {
        ...
        let regular = FontRef::from_index(&sfns, 0).expect("SFNS.ttf has a font at index 0");
        let italic =
            FontRef::from_index(&sfns_italic, 0).expect("SFNSItalic.ttf has a font at index 0");
