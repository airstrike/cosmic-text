...
impl Font {
    ...
    /// Per-font decoration metrics (underline / strikethrough offset +
    /// thickness, and ascent), in EM units.
    ///
    /// This is the reshape-free part of decoration geometry: it depends only
    /// on the font, so a render-time resolver can cache the result per
    /// `font_id` and scale by the glyph font size at draw — no shaping needed.
    /// Falls back to sensible defaults for fonts that omit the metrics.
